# File: Final LMTP analysis - Ethiopia
# Date: 07/25/2023
...
#### 1. LOAD DATA  ####
#------------------------------------------------------------------------------
df1 <- read.csv("ethiopia_final_data.csv", sep = ";")
commid <- read.csv("ethiopia_commid.csv", sep = ";")

#------------------------------------------------------------------------------
#### 2. IMPUTATION  ####
#------------------------------------------------------------------------------
df1 <- df1 %>%
  select(-c(chotherrel,hadiva,gurage,oromo,sidama,tigrian,wolavta,L2_proscrm,L1_proscrm)) %>%
  distinct() %>%
  mutate_at(c("careage","wellbeing","hhsize","L1_pop","A1","L2_pop","A2","gad7score",
              "phq8score"), as.numeric) %>%
  mutate_at(c("childid","ever_epidemic","momdied","chsex","mightdie","fatherill",
              "motherill","divorce","alcohol","foodsec","ownland","jobloss",
              "chmuslim","chorthodox","amhara",
              "carenone","carerel","L1_sochel","L1_thfcrm",
              "L2_sochel","L2_thfcrm"), as.factor)
...
meth_analysis = c("","logreg","","","logreg","pmm","logreg","logreg",
                  "logreg","logreg","pmm","pmm","logreg","polyreg",
...
#Reorder
gad <- gad %>%
  select(c(.imp,.id,commid,ever_epidemic,momdied,chsex,mightdie,careage,
           fatherill,motherill,divorce,alcohol,wellbeing,hhsize,foodsec,
           ownland,jobloss,chmuslim,chorthodox,amhara,carenone,carerel,
           L1_sochel,L1_pop,L1_thfcrm,A1,C1,L2_sochel,L2_pop,L2_thfcrm,
           A2,C2,Y)) 

gad_girls <- gad %>%
  filter(chsex == 0) %>% 
  select(c(.imp,.id,commid,ever_epidemic,momdied,mightdie,careage,
           fatherill,motherill,divorce,alcohol,wellbeing,hhsize,foodsec,
           ownland,jobloss,chmuslim,chorthodox,amhara,carenone,carerel,
           L1_sochel,L1_pop,L1_thfcrm,A1,C1,L2_sochel,L2_pop,L2_thfcrm,
           A2,C2,Y)) 
...
W <- list(trt = c("foodsec","ownland"), 
          cens = c("ever_epidemic","momdied","chsex","mightdie","careage",
                   "fatherill","motherill","divorce",
                   "alcohol","wellbeing","hhsize","foodsec","ownland","jobloss",
                   "chmuslim","chorthodox","amhara","carenone","carerel"),
          outcome = c("ever_epidemic","momdied","chsex","mightdie","careage",
                      "fatherill","motherill","divorce",
                      "alcohol","wellbeing","hhsize","foodsec","ownland","jobloss",
                      "chmuslim","chorthodox","amhara","carenone","carerel"))

W_gender <- list(trt = c("foodsec","ownland"), 
                 cens = c("ever_epidemic","momdied","mightdie","careage",
                          "fatherill","motherill","divorce",
                          "alcohol","wellbeing","hhsize","foodsec","ownland","jobloss",
                          "chmuslim","chorthodox","amhara","carenone","carerel"),
                 outcome = c("ever_epidemic","momdied","mightdie","careage",
                             "fatherill","motherill","divorce",
                             "alcohol","wellbeing","hhsize","foodsec","ownland","jobloss",
                             "chmuslim","chorthodox","amhara","carenone","carerel"))
...
L <- list(trt = list(c(), 
                     c()), 
          cens = list(c("L1_sochel","L1_pop","L1_thfcrm"), 
                      c("L2_sochel","L2_pop","L2_thfcrm")), 
          outcome = list(c("L1_sochel","L1_pop","L1_thfcrm"), 
                         c("L2_sochel","L2_pop","L2_thfcrm")))
