...
#### 1. LOAD DATA  ####
#------------------------------------------------------------------------------
df1 <- read.csv("peru_final_data.csv", sep = ";")
commid <- read.csv("peru_commid.csv", sep = ";")

#------------------------------------------------------------------------------
#### 2. IMPUTATION  ####
#------------------------------------------------------------------------------
df1 <- df1 %>%
  select(-c(disability, alcohol, white, other,chother)) %>%
  distinct() %>%
  mutate_at(c("careage","wellbeing","hhsize","L1_pop","A1","L2_pop","A2","gad7score_2",
              "phq8score_2"), as.numeric) %>%
  mutate_at(c("childid","chsex","mightdie","fatherill","motherill",
              "smoking","ever_epidemic","carenone","divorce","chnone","ownland",
              "ownanimals","foodsec","ownhouse",
              "jobloss","L1_thfcrm","L2_thfcrm","L1_violcrm","L2_violcrm",
              "L1_yuthcrm","L2_yuthcrm"), as.factor)
...
meth_analysis = c("","logreg","","","pmm","logreg","logreg","logreg","logreg",
                  "pmm","pmm","logreg","logreg","logreg","polyreg","logreg",
                  "logreg","logreg","pmm","logreg","logreg","logreg","",
...
imp.combined <- imp.combined %>% 
  drop_na(commid) #remove observations missing a community ID

#Separate by outcomes
gad <- imp.combined %>%
  rename(Y = gad7score_2) %>%
  select(-phq8score_2) %>%
  distinct()

phq <- imp.combined %>%
  rename(Y = phq8score_2) %>%
  select(-gad7score_2) %>%
  distinct()
...
#Reorder
gad <- gad %>%
  select(c(.imp,.id,commid,ever_epidemic,chsex,mightdie,careage,
           fatherill,motherill,divorce,smoking,wellbeing,hhsize,
           ownland,ownanimals,ownhouse,foodsec,jobloss,chnone,
           carenone,L1_pop,L1_thfcrm,L1_yuthcrm,L1_violcrm,A1,C1,
           L2_pop,L2_thfcrm,L2_yuthcrm,L2_violcrm,A2,C2,Y)) 

gad_girls <- gad %>%
  filter(chsex == 0) %>% 
  select(c(.imp,.id,commid,ever_epidemic,mightdie,careage,
           fatherill,motherill,divorce,smoking,wellbeing,hhsize,
           ownland,ownanimals,ownhouse,foodsec,jobloss,chnone,
           carenone,L1_pop,L1_thfcrm,L1_yuthcrm,L1_violcrm,A1,C1,
           L2_pop,L2_thfcrm,L2_yuthcrm,L2_violcrm,A2,C2,Y)) 
...
W <- list(trt = c("ownland","ownanimals","ownhouse","foodsec"), 
          cens = c("ever_epidemic","chsex","mightdie","careage","fatherill",
                   "motherill","divorce","smoking","wellbeing","hhsize",
                   "ownland","ownanimals","ownhouse","foodsec","jobloss",
                   "chnone","carenone"),
          outcome = c("ever_epidemic","chsex","mightdie","careage","fatherill",
                      "motherill","divorce","smoking","wellbeing","hhsize",
                      "ownland","ownanimals","ownhouse","foodsec","jobloss",
                      "chnone","carenone"))

W_gender <- list(trt = c("ownland","ownanimals","ownhouse","foodsec"), 
                 cens = c("ever_epidemic","mightdie","careage","fatherill",
                          "motherill","divorce","smoking","wellbeing","hhsize",
                          "ownland","ownanimals","ownhouse","foodsec","jobloss",
                          "chnone","carenone"),
                 outcome = c("ever_epidemic","mightdie","careage","fatherill",
                             "motherill","divorce","smoking","wellbeing","hhsize",
                             "ownland","ownanimals","ownhouse","foodsec","jobloss",
                             "chnone","carenone"))
...
L <- list(trt = list(c(), 
                     c()), 
          cens = list(c("L1_pop","L1_yuthcrm","L1_violcrm","L1_thfcrm"), 
                      c("L2_pop","L2_yuthcrm","L2_violcrm","L2_thfcrm")), 
          outcome = list(c("L1_pop","L1_yuthcrm","L1_violcrm","L1_thfcrm"), 
                         c("L2_pop","L2_yuthcrm","L2_violcrm","L2_thfcrm")))
