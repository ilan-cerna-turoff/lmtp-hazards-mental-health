# File: Final LMTP analysis - Vietnam
# Date: 07/20/2023
...
#### 1. LOAD DATA  ####
#------------------------------------------------------------------------------
df1 <- read.csv("vietnam_final_data.csv", sep = ";")
commid <- read.csv("vietnam_commid.csv", sep = ";")

#------------------------------------------------------------------------------
#### 2. IMPUTATION  ####
#------------------------------------------------------------------------------
df1 <- df1 %>%
  select(-c(carerel,disability,minorityethnic,kinh,L1_thfcrm,L2_thfcrm,alcohol)) %>%
  distinct() %>%
  mutate_at(c("careage","wellbeing","hhsize","L1_pop","A1","L2_pop","A2","gad7score",
              "phq8score"), as.numeric) %>%
  mutate_at(c("childid","ever_epidemic","chsex","mightdie","fatherill","motherill","divorce",
              "smoking","foodsec","ownland","chbuddhist","minorityrel","education",
              "carenone","L1_proscrm","L1_yuthcrm","L1_violcrm",
              "L2_proscrm","L2_yuthcrm","L2_violcrm"), as.factor)
...
meth_analysis = c("","logreg","","","","logreg","logreg","logreg","logreg","logreg",
                  "pmm","pmm","logreg","polyreg","logreg","logreg","logreg","",
...
#Reorder
gad <- gad %>%
  select(c(.imp,.id,commid,ever_epidemic,mightdie,careage,chsex,
           fatherill,motherill,divorce,smoking,
           wellbeing,hhsize,foodsec,ownland,chbuddhist,minorityrel,
           carenone,education,L1_pop,L1_proscrm,L1_yuthcrm,
           L1_violcrm,A1,C1,L2_pop,L2_proscrm,
           L2_yuthcrm,L2_violcrm,A2,C2,Y)) 
...
W <- list(trt = c("foodsec","ownland"), 
          cens = c("ever_epidemic",
                   "chsex","mightdie","careage","fatherill",
                   "motherill","divorce",
                   "smoking","wellbeing","hhsize","foodsec","ownland",
                   "chbuddhist","minorityrel",
                   "carenone","education"),
          outcome = c("ever_epidemic",
                      "chsex","mightdie","careage","fatherill",
                      "motherill","divorce",
                      "smoking","wellbeing","hhsize","foodsec","ownland",
                      "chbuddhist","minorityrel",
...
L <- list(trt = list(c(), 
                     c()), 
          cens = list(c("L1_pop","L1_proscrm","L1_yuthcrm","L1_violcrm"), 
                      c("L2_pop","L2_proscrm","L2_yuthcrm","L2_violcrm")), 
          outcome = list(c("L1_pop","L1_proscrm","L1_yuthcrm","L1_violcrm"), 
                         c("L2_pop","L2_proscrm","L2_yuthcrm","L2_violcrm")))
